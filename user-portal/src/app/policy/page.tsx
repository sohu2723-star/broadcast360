    ...
    <main className="min-h-screen bg-[#071029] px-4 py-10 text-white sm:px-6">
      <div className="mx-auto max-w-3xl rounded-[2rem] border border-[#7898bf]/15 bg-[#101a3a]/95 p-6 shadow-[0_30px_90px_rgba(0,0,0,0.38)] sm:p-10">
        ...
        <div className="mt-8 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-[#a9c0dd]/70">Hxu Movie</p>
            <h1 className="mt-1 text-3xl font-bold">Acceptable Use & Privacy Policy</h1>
          </div>
        </div>
        <p className="mt-8 text-sm leading-7 text-slate-300">By creating and using a Hxu Movie account, you agree to use the service lawfully, protect your account credentials, and provide accurate account information.</p>
        <div className="mt-8 space-y-7 text-sm leading-7 text-slate-300">
          ...
          <section><h2 className="text-lg font-semibold text-white">Content and service use</h2><p className="mt-2">Use Hxu Movie only for lawful viewing and account activity. Do not upload, distribute, or access content in a way that violates applicable law or the rights of others.</p></section>
          ...
          <section><h2 className="text-lg font-semibold text-white">Contact</h2><p className="mt-2">For account questions or reactivation requests, use the Support area in Hxu Movie.</p></section>
        </div>
        <div className="mt-10 rounded-2xl border border-[#7898bf]/15 bg-[#0b1636]/65 p-4 text-xs leading-6 text-slate-400">Policy version: August 2026. This page is provided as the current Hxu Movie service policy and may be updated as the service evolves.</div>
      </div>
    </main>
  );
}
