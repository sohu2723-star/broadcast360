...
export default async function NewsDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const item = Number.isInteger(newsId) && newsId > 0
    ? await prisma.news.findUnique({ where: { id: newsId }, include: { channel: true } })
    : null;

  if (!item) {
    return <main className="min-h-screen bg-[#010312] p-8 text-white"><Link href="/admin/news" className="inline-flex items-center gap-2 text-sm text-[#c5d7ee]"><ArrowLeft size={16} /> Back to News</Link><div className="mt-8 rounded-2xl border border-white/10 bg-[#0B1026] p-8">News item not found.</div></main>;
  }
