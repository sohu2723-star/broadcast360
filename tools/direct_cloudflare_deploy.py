...
WORKER_REQUIRE_SHIM = r'''var __flickscopeRequire = globalThis.__flickscopeRequire || ((id) => {
  ...
  const EventEmitter = class { constructor() { this._listeners = {}; } on(name, fn) { (this._listeners[name] ||= []).push(fn); return this; } once(name, fn) { const once = (...args) => { this.off(name, once); fn(...args); }; return this.on(name, once); } off(name, fn) { this._listeners[name] = (this._listeners[name] || []).filter((item) => item !== fn); return this; } removeListener(name, fn) { return this.off(name, fn); } addListener(name, fn) { return this.on(name, fn); } removeAllListeners(name) { if (name) delete this._listeners[name]; else this._listeners = {}; return this; } emit(name, ...args) { for (const fn of this._listeners[name] || []) fn(...args); return (this._listeners[name] || []).length > 0; } }; EventEmitter.EventEmitter = EventEmitter; EventEmitter.defaultMaxListeners = 10; const events = EventEmitter;
